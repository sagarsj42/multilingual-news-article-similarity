...
def collate_fn(batch, tokenizer):
    ...
    for sample in batch:
        text1 = str(sample['ner_1']).lower().strip() +                 str(sample['title_1']).lower().strip() + str(sample['meta_description_1']).lower().strip() +             ' '.join(sample['meta_keywords_1']).lower().strip() + ' '.join(sample['tags_1']).lower().strip()
        text2 = str(sample['ner_2']).lower().strip() +                 str(sample['title_2']).lower().strip() + str(sample['meta_description_2']).lower().strip() +             ' '.join(sample['meta_keywords_2']).lower().strip() + ' '.join(sample['tags_2']).lower().strip()
